...
/*   Updated: 2023/02/22 20:51:07 by mrezaei          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	find_min_new(t_stack *s, int start, int end)
{
	int	min;
	int	i;
...
int	find_max_new(t_stack *s, int start, int end)
{
	int	max;
	int	i;
...
int	get_pivot_new(t_stack *a)
{
	printf("min=[%d]\n", find_min(a));
	printf("max=[%d]\n", find_max(a));
	return (find_min(a) + ((find_max(a) - find_min(a)) / 2));
}
...
void	sort_chunk(t_stack *a, t_stack *b, int start, int end)
{
	...
	while (i < a->size)
	{
		if (a->t_stack[a->top] < pivot)
			pb(a, b);
		else if (a->t_stack[a->top] >= pivot && a->t_stack[a->top] < (2 * pivot))
		{
			pb(a, b);
			if (b->t_stack[b->top] <)
			rrb(b);
	...
	// }
	i = 0;
	while (i < (a->size - 2 * pivot))
	{
		if (a->t_stack[a->top] >= (2 * pivot) && a->t_stack[a->top] < (3 * pivot))
			pb(a, b);
		else
			ra(a);
		i++;
	...
	i = 0;
	while (i < (a->size - 3 * pivot))
	{
		if (a->t_stack[a->top] >= (3 * pivot) && a->t_stack[a->top] < (4 * pivot))
			pb(a, b);
		else
			ra(a);
		i++;
	...
	while (is_empty(b) != -1)
	{
		max = find_max(b);
		max_i = max_index(b);
		if (abs(max_i - b->top) < 20)
			rb(b);
		else
			rrb(b);
		if (b->t_stack[b->top] == max)
			pa(a, b);
	// ptest(a);
	// ptest(b);
	} 
...
void	big_sort(t_stack *a, t_stack *b, int start, int end)
{
	int	i;
	int	tmp;
	t_stack	pivot;

	init_stack(&pivot, end);
	if (end - start < 6)
		return ;
	if (start > end)
		return ;
	while (!is_sorted(a))
	{
		...
		if (a->top >= 3)
		{
			...
			while (i)
			{
				if (a->t_stack[a->top] < tmp)
					pb(a, b);
				else if (a->t_stack[a->top] >= tmp)
				{
					if (a->t_stack[0] < tmp)
					{
						rra(a);
...
//	printf("pivot_stack=[%d]", pivot.t_stack[pivot.top]);
	ptest(&pivot);
	i = 0;
	tmp = pivot.t_stack[i] - pivot.t_stack[i + 1];
	if (tmp == 2 && b->t_stack[b->top] < b->t_stack[b->top - 1])
		sb(b);
	pa(a, b);
	pa(a, b);
	tmp = pivot.t_stack[i + 1] - pivot.t_stack[i + 2];
	if (tmp == 3)
	{
		int v[3];

		v[0] = b->t_stack[b->top];
		v[1] = b->t_stack[b->top - 1];
		v[2] = b->t_stack[b->top - 2];
		...
		else if (v[2] > v[1] && v[0] < v[1] && v[2] > v[0])
		{
			pa(a, b);
			sb(b);
			pa(a, b);
		...
		else if (v[1] > v[0] && v[1] > v[2] && v[0] > v[2])
		{
			sb(b);
			pa(a, b);
			pa(a, b);
		...
		else if (v[1] > v[0] && v[1] > v[2] && v[2] > v[0])
		{
			sb(b);
			pa(a, b);
			sb(b);
	...
	ptest(a);
	ptest(b);
	tmp = pivot.t_stack[i + 2] - pivot.t_stack[i + 3];
	if (tmp > 3 && tmp <= 6)
	{
		int new_piv;
		int	max;
		max = find_max_new(b, b->top - tmp + 1, b->top);
		printf("min===[%d]", max);
		new_piv = max - tmp / 2;
		i = 0;
		while (i < tmp)
		{
			if (b->t_stack[b->top] > new_piv)
				pa(a, b);
			else
				rb(b);
			i++;
	...
	printf("b=");
	ptest(b);
	// big_sort(a, b, start, pivot - 1);
	// big_sort(a, b, a->size / 2, end);
}
...
void	ft_sort(t_stack *a, t_stack *b)
{
	...
	{
		ft_repleace(a, b);
		sort_chunk(a, b, 0, a->size);
		//big_sort(a, b, 0, a->size);
		// quicksort(a, b);
	}
	return ;
}
...
int	main(int argc, char **argv)
{
	t_stack	a;	
	t_stack	b;
	char *my_argv[] = {"program_name", "68 19 60 89 47 2 12 92 41 49 36 11 53 72 26 95 8 71 32 45 28 9 50 33 35 99 8998989 -8484 37 25 98494 5494 80 88 96 455454 30 42 22 79 87 61 90 76 38 1 20 17 44 91 59 46 69 23 83 54 15 8414 55 98 43 18 84881 16 85 24 77 70 31 1463 29 78 65 -57 10 56 97 3 67 52 39 63 75 455640 -21 58 6-544 66 82 -545451 48 73 256457 9844 4 7 62 -65434 86 -5"};
